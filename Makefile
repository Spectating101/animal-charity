...
replay-smoke:
	...
	@python scripts/replay_public_good_case.py examples/replay_disaster_krayan_2026_07_17_dependency.json --score | python -c "import json,sys; d=json.load(sys.stdin); assert d['predicted_primary_stage']=='access'; assert d['primary_stage_match'] is True"
	@python scripts/replay_public_good_case.py examples/replay_disaster_nunukan_2026_06_02_structural_access.json --score | python -c "import json,sys; d=json.load(sys.stdin); assert d['predicted_primary_stage']=='access'; assert d['primary_stage_match'] is True; assert d['predicted_structural_candidate'] is True"
	@python scripts/replay_public_good_case.py examples/replay_disaster_humbang_2025_11_30_conditional_route.json --score | python -c "import json,sys; d=json.load(sys.stdin); assert d['predicted_primary_stage']=='safety'; assert d['primary_stage_match'] is True; assert d['predicted_structural_candidate'] is False"

replay-corpus-smoke:
	@python scripts/evaluate_replay_corpus.py config/research/disaster_replay_corpus.json | python -c "import json,sys; d=json.load(sys.stdin); assert d['case_count']==14; assert d['passed_count']==14; assert d['failed_count']==0; assert d['by_evidence_level']=={'R1': 14}; assert any(x['case_id']=='nunukan-access-2026-06-02-structural-public' and x['predicted_structural_candidate'] is True for x in d['results']); assert any(x['case_id']=='humbang-2025-11-30-conditional-route-public' and x['problem_classes']==['conditional_operational_safety_constraint'] and x['finding_count']==1 and x['resource_reservation_count']==0 for x in d['results'])"
